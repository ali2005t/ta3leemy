...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentTeacherId = null; // Added for robust ID handling
    ...
    onAuthStateChanged(auth, async (user) => {
        if (user) {
            ...
            if (uid) {
                currentTeacherId = uid; // Store effective ID
                // Initialize page
                loadCodes(uid);
                loadTrainings(uid); // Fix: Load Trainings
            }
        } else {
    ...
    document.getElementById('generate-codes-form').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            loadCodes(currentTeacherId); // Will need update to handle loading efficiently if lists get huge
        ...
        filtered.forEach(data => {
            ...
            tr.innerHTML = `
                ...
                <td style="font-size:0.85rem; color:#64748b;">
                    ${data.usedByName ? data.usedByName : (data.usedBy ? 'Student ID: ' + data.usedBy.substr(0, 5) : '-')}
                </td>
    ...
    if (delAllBtn) delAllBtn.onclick = async () => {
        ...
        if (!await UIManager.showConfirm("حذف جماعي", `هل أنت متأكد من حذف ${toDelete.length} كود؟`)) return;
        ...
        try {
            ...
            loadCodes(currentTeacherId); // Reload
    ...
    function translatePlan(p) {
        if (p === 'basic') return 'المعلم';
        if (p === 'pro') return 'السنتر';
        if (p === 'elite') return 'الأكاديمية';
        return p;
    }
    ...
    async function renderPrintCards() {
        ...
        if (currentTeacherId) {
            try {
                const d = await getDoc(doc(db, "teachers", currentTeacherId));
                if (d.exists()) {
                    const t = d.data();
