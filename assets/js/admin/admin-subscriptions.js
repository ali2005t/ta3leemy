...
window.updateSubPrice = () => {
    ...
    const tierLimits = {
        'basic': 100,
        'pro': 1000,
        'elite': '' // user wants empty for unlimited
    };

    // Auto-set limit if empty or changing tiers (optional UX, let's just set it)
    // Only set if user hasn't typed? No, let's just update it for convenience
    studentInput.value = tierLimits[tier];
    ...
    if (months >= 12) total = total * 0.8; // 20% off yearly - 2 months free roughly
    else if (months >= 6) total = total * 0.9;
