...
// RBAC Check
const adminRole = 'super_admin'; // Forced Permissions
...
async function loadTeachers() {
    ...
    try {
        ...
        snapshot.forEach(docSnap => {
            ...
            // ... Inside loop ...
            const teacherCode = data.code || docSnap.id.substring(0, 6).toUpperCase();

            const tr = document.createElement('tr');
            tr.dataset.search = `${data.name} ${data.email} ${data.platformName || ''} ${teacherCode} ${data.phone || ''}`.toLowerCase();
            ...
            tr.innerHTML = `
                <td>
                    ...
                    <div style="font-size:0.75rem; color:#f59e0b; font-family:monospace; letter-spacing:1px; margin-bottom:2px;">Code: ${teacherCode}</div>
                    <div style="font-size:0.8rem; opacity:0.8;">${data.phone || '-'}</div>
                     ${(data.assignedAgentName && data.assignedAgentName !== 'undefined') ? `<div style="font-size:0.7rem; color:#10b981; margin-top:2px;"><i class="fas fa-shield-alt"></i> ${data.assignedAgentName}</div>` : ''}
                </td>
                <td>${data.email}</td>
        ...
        async function exportTeachersData() {
            try {
                ...
                const rows = [];
                rows.push(["Name", "Email", "Phone", "PlatformName", "Status", "Date"].join(","));

                snap.forEach(doc => {
                    const d = doc.data();
                    const date = d.createdAt ? new Date(d.createdAt.seconds * 1000).toLocaleDateString('en-CA') : '';
                    const row = [
                        `"${d.name || ''}"`,
                        `"${d.email || ''}"`,
                        `"${d.phone || ''}"`,
...
window.openViewProfileModal = async (tid) => {
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            // Inject Assign Button into UID section
            document.getElementById('vp-uid').innerHTML = `${tid} <br> 
            <button onclick="openAssignModal('${tid}', '${data.name}')" class="btn-sm" style="margin-top:8px; padding:4px 12px; background:#3b82f6; border:none; color:white; border-radius:4px; cursor:pointer; font-size:0.85rem;">
                <i class="fas fa-user-tag"></i> تعيين / تغيير المساعد
            </button>`;
...
async function loadFinancialHistory(tid) {
    ...
    const balanceEl = document.getElementById('fin-balance-display');
    ...
    try {
        ...
        if (snapshot.empty) {
            ...
            if (balanceEl) balanceEl.innerText = "الرصيد: 0.00 ج.م";
            return;
        }
        ...
        let totalIncome = 0;
        let totalPaid = 0; // Deductions + Payments to teacher effectively
        let balance = 0;
        ...
        snapshot.forEach(doc => {
            ...
            if (data.type === 'invoice') {
                typeBadge = '<span class="status-badge" style="background:#e0e7ff; color:#4338ca;">فاتورة</span>';
                balance -= amt; // Teacher owes us
            }
            else if (data.type === 'payment') {
                typeBadge = '<span class="status-badge" style="background:#dcfce7; color:#166534;">إضافة (له)</span>';
                balance += amt; // Added to his wallet
            }
            else if (data.type === 'deduction') {
                typeBadge = '<span class="status-badge" style="background:#fee2e2; color:#991b1b;">خصم (عليه)</span>';
                balance -= amt; // Deduced from him
            }

            tbody.innerHTML += `
                <tr>
                    <td>${typeBadge}</td>
                    <td style="font-weight:bold; direction:ltr;">${amt.toFixed(2)}</td>
                    <td>${data.reason}</td>
                    <td>${date}</td>
        ...
        // Update Balance Display if exists, else inject it
        if (!balanceEl) {
            const header = document.querySelector('#financials-modal h3');
            const balDiv = document.createElement('span');
            balDiv.id = 'fin-balance-display';
            balDiv.style.marginRight = '20px';
            balDiv.style.fontSize = '0.9rem';
            balDiv.style.color = balance >= 0 ? '#10b981' : '#ef4444';
            balDiv.innerText = `الرصيد: ${balance.toFixed(2)} ج.م`;
            header.appendChild(balDiv);
        } else {
            balanceEl.style.color = balance >= 0 ? '#10b981' : '#ef4444';
            balanceEl.innerText = `الرصيد: ${balance.toFixed(2)} ج.م`;
        }
...
window.copyProfileLink = (url) => {
    navigator.clipboard.writeText(url).then(() => {
        UIManager.showToast('تم نسخ رابط المعلم بنجاح');
    }).catch(err => {
        // Fallback
        ...
        textArea.remove();
        UIManager.showToast('تم نسخ الرابط');
    });
};

document.getElementById('vp-link-go').addEventListener('click', (e) => {
    e.preventDefault();
    const url = document.getElementById('vp-link').value;
    window.open(url, '_blank');
});

// Add Copy Button Listener Integration
// We'll replace the input click to copy
document.getElementById('vp-link').addEventListener('click', function () {
    window.copyProfileLink(this.value);
});


// List Referrals Logic
window.viewReferralsList = async (tid) => {
    // Create a simple modal on the fly or reuse one
    // For simplicity, we alert names or use a custom small prompt
    // Better: Replace the count text with a list momentarily

    const container = document.getElementById('vp-referral-count').parentElement;
    const originalContent = container.innerHTML;

    container.innerHTML = '<i class="fas fa-spinner fa-spin"></i> جاري التحميل...';

    try {
        const q = query(collection(db, "teachers"), where("referredBy", "==", tid));
        const snap = await getDocs(q);

        if (snap.empty) {
            alert("لم يتم دعوة أي معلم بعد.");
            container.innerHTML = originalContent;
            return;
        }

        let msg = "المعلمون الذين تمت دعوتهم:\n";
        snap.forEach(d => {
            const t = d.data();
            msg += `- ${t.name} (${t.platformName || '-'}) \n`;
        });

        alert(msg); // Simple list for now as requested "Know who"
...
// Add clickable to count
document.getElementById('vp-referral-count').style.cursor = 'pointer';
document.getElementById('vp-referral-count').style.textDecoration = 'underline';
document.getElementById('vp-referral-count').onclick = function () {
    const tid = document.getElementById('vp-uid').innerText;
    window.viewReferralsList(tid);
};
